...
// Dither mode: 0 = RAW (off), 1 = Bayer ordered, 2 = Floyd-Steinberg
// All dithering happens in WAT; harness just writes mode to control block byte 0x11
let ditherMode = 1;
...
function blitFramebuffer() {
  ...
  // Simple palette blit — all dithering is handled in WAT
...
function loop(ts) {
  ...
  // Dither mode for WASM: 0=RAW, 1=Bayer, 2=Floyd-Steinberg (all handled in WAT)
  memU8[CTL_OFFSET + 17] = ditherMode;
